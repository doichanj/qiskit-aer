...
public:
  ...
  // Return the current value of the memory as little-endian hex-string
  inline std::string register_hex() const {
    return Utils::bin2hex(creg_register_);
  }
...
void ClassicalRegister::initialize(size_t num_memory, size_t num_register,
  ...
  creg_register_ =
      std::move(Utils::padleft_inplace(memory_bin, '0', num_register));
}

void ClassicalRegister::store_measure(const reg_t &outcome, const reg_t &memory,
                                      const reg_t &registers) {
...
void ClassicalRegister::apply_bfunc(const Operations::Op &op) {
  ...
  if (op.type != Operations::OpType::bfunc) {
    throw std::invalid_argument(
        "ClassicalRegister::apply_bfunc: Input is not a bfunc op.");
  }

  const std::string &mask = op.string_params[0];
  const std::string &target_val = op.string_params[1];
  int_t compared; // if equal this should be 0, if less than -1, if greater than
                  // +1

  // Check if register size fits into a 64-bit integer
  if (creg_register_.size() <= 64) {
    uint_t reg_int =
        std::stoull(creg_register_, nullptr, 2);      // stored as bitstring
    uint_t mask_int = std::stoull(mask, nullptr, 16); // stored as hexstring
    uint_t target_int =
        std::stoull(target_val, nullptr, 16); // stored as hexstring
    compared = (reg_int & mask_int) - target_int;
  } else {
    // We need to use big ints so we implement the bit-mask via the binary
    // string representation rather than using a big integer class
    ...
    for (size_t rev_pos = 0; rev_pos < length; rev_pos++) {
      masked_val[length - 1 - rev_pos] =
          (mask_bin[mask_bin.size() - 1 - rev_pos] &
...
// Apply readout error instruction to classical registers
void ClassicalRegister::apply_roerror(const Operations::Op &op,
                                      RngEngine &rng) {

  // Check input is readout error op
  if (op.type != Operations::OpType::roerror) {
    throw std::invalid_argument(
        "ClassicalRegister::apply_roerror Input is not a readout error op.");
  }
  ...
  for (size_t pos = 0; pos < op.memory.size(); ++pos) {
    ...
    creg_memory_[creg_memory_.size() - 1 - bit] =
