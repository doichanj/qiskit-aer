...
/*
    auto measure_determinisitic_func = [this, qubit, &exponent_l, &exponent_c, &exponent_h, blocks, blockSize](AER::int_t q)
    {
      bool accumX = false;
      bool accumZ = false;
      ...
      for (uint64_t ii = 0; ii < blocks; ii++) {
        ...
        uint64_t ax = destabilizer_mask & sX;
        uint64_t az = destabilizer_mask & sZ;
        for(int b=1;b<blockSize;b*=2){
          ax ^= (ax << b);
          az ^= (az << b);
        }
        ax ^= (0ull - (uint64_t)accumX);
        az ^= (0ull - (uint64_t)accumZ);

        accumX = (ax >> (blockSize - 1)) == 1;
        accumZ = (az >> (blockSize - 1)) == 1;
        ...
        if(q == 0)
          local_exponent_h ^= (stabilizer_phases_(ii) & destabilizer_mask);
      }
      return AER::Utils::popcount(local_exponent_l) + 2*AER::Utils::popcount(local_exponent_h);
    };
    uint_t count = AER::Utils::apply_omp_parallel_for_reduction((num_qubits_ > omp_threshold_ && omp_threads_ > 1 && nid == 1), 0, num_qubits_, measure_determinisitic_func, omp_threads_);
