...
template <class clifford_t = Clifford::Clifford>
class State : public QuantumState::State<clifford_t> {
public:
  using BaseState = QuantumState::State<clifford_t>;
...
template <class clifford_t>
const stringmap_t<Gates> State<clifford_t>::gateset_({
  // Single qubit gates
...
template <class clifford_t>
void State<clifford_t>::initialize_qreg(uint_t num_qubits) 
{
...
template <class clifford_t>
size_t State<clifford_t>::required_memory_mb(uint_t num_qubits,
                                 const std::vector<Operations::Op> &ops)
                                 const  {
...
template <class clifford_t>
void State<clifford_t>::set_config(const Config &config) {
...
template <class clifford_t>
void State<clifford_t>::apply_op(const Operations::Op &op,
                     ExperimentResult &result,
...
template <class clifford_t>
void State<clifford_t>::apply_gate(const Operations::Op &op) {
...
template <class clifford_t>
void State<clifford_t>::apply_pauli(const reg_t &qubits, const std::string& pauli) {
...
template <class clifford_t>
void State<clifford_t>::apply_measure(const reg_t &qubits,
                          const reg_t &cmemory,
                          const reg_t &cregister,
                          RngEngine &rng) {
...
template <class clifford_t>
void State<clifford_t>::apply_reset(const reg_t &qubits, RngEngine &rng) {
  ...
  for (size_t j=0; j < qubits.size(); j++) {
    if (outcome[j] == 1) {
      BaseState::qreg_.append_x(qubits[j]);
    }
  }
}
...
template <class clifford_t>
reg_t State<clifford_t>::apply_measure_and_update(const reg_t &qubits,
                                      RngEngine &rng) {
  ...
  for (const auto &q : qubits) {
    ...
    outcome.push_back(BaseState::qreg_.measure_and_update(q, r));
  }
  return outcome;
}

template <class clifford_t>
std::vector<reg_t> State<clifford_t>::sample_measure(const reg_t &qubits,
                                         uint_t shots,
                                         RngEngine &rng) {
...
template <class clifford_t>
void State<clifford_t>::apply_set_stabilizer(const Clifford::Clifford &clifford) {
...
template <class clifford_t>
void State<clifford_t>::apply_save_stabilizer(const Operations::Op &op,
                                ExperimentResult &result) {
  ...
  result.save_data_pershot(BaseState::creg(), key, std::move(clifford), op_type, op.save_type);
}

template <class clifford_t>
void State<clifford_t>::apply_save_probs(const Operations::Op &op,
                             ExperimentResult &result) {
...
template <class clifford_t>
void State<clifford_t>::apply_save_amplitudes_sq(const Operations::Op &op,
                                     ExperimentResult &result) {
...
template <class clifford_t>
double State<clifford_t>::expval_pauli(const reg_t &qubits,
...
template <class clifford_t>
template <typename T>
void State<clifford_t>::get_probabilities_auxiliary(const reg_t &qubits,
                                        std::string outcome,
                                        double outcome_prob,
                                        T &probs) {
...
template <class clifford_t>
double State<clifford_t>::get_probability(const reg_t &qubits, const std::string &outcome) {
...
template <class clifford_t>
void State<clifford_t>::get_probability_helper(const reg_t &qubits,
                                   const std::string &outcome,
                                   std::string &outcome_carry,
                                   double &prob_carry) {
  uint_t qubit_for_branching = -1;
